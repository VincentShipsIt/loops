...
Next-24h flow gate:
- If a `Next 24h Queue Planner` automation is configured, read its latest successful memory after the Backlog preflight and before repository synchronization or issue selection.
- Honor `hold_new_implementation: true` only when it names this exact repository and its planner timestamp is less than 24 hours old. Report the planner's open-PR/conflict evidence and stop without claiming, project mutation, repository synchronization, branch creation, or code writes.
- A hold is a run-level no-op, never a scheduler pause. Missing, stale, blocked, or ambiguous planner memory has no hold authority; continue with the normal eligibility gates.
...
- Never run a full-repository or full-monorepo test, typecheck, build, coverage, or end-to-end suite on the local computer. Locally run only focused package/file checks that repository instructions explicitly permit. Run every broad suite through GitHub Actions.
- Before a push or workflow dispatch that would start broad CI, acquire the configured fleet-wide CI serialization lock and verify that no other fleet repository has a queued or running broad validation workflow. Hold the lock until the required GitHub checks finish, then release it even on failure. Wait rather than starting a second broad run; never bypass a live lock. Recover a stale lock only when its owner is gone and GitHub confirms that no fleet-wide broad run is active.
- Run the relevant formatter, lint, focused unit checks, and user-facing checks discovered from repository instructions and scripts, proportional to the change. Record the exact local or GitHub Actions command and blocker for any required check that cannot run.
