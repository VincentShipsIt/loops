...
| `pr-review` | Review one current PR head; optionally repair the same automation-owned branch. | Review comments/marker; gated same-branch commit only. | Isolated checkout, ownership/small-fix gates, marker with automation id and head SHA. | Failed verification blocks repair; failed marker write is reported and not deduped. |
